...
\item The final link driver is chosen the same way real
  \verb|bsd.init.mk| chooses its internal \verb|_CCLINK| — \verb|${CXX}|
  when the module has a C++ source, \verb|${CC}| otherwise — reimplemented
  as Bmake It's own \verb|_CCLINK|, since \verb|mk.prog.mk|/\verb|mk.lib.mk|
  never \verb|.include| any real \verb|bsd.*.mk| file to inherit it
  from\reqfoot{REQ-cxx-link-driver-selection-req}.
\end{itemize}

New, project-specific concepts (\verb|PARENT_WS|, \verb|PREREQS|,
\verb|PROG|, \verb|LIB|, \verb|LIBS|, \verb|LIB_SHARED|, \verb|INCL|,
\verb|TOOLCHAIN|, \verb|LINK_CXX|) exist only for domain concepts BSD
make has no native equivalent for at all — workspace parenting,
framework prerequisites, module identity, linking, header promotion,
toolchain bundle selection, and the one link-driver case
\verb|SRCS|-based auto-detection cannot see (a C-only module linking a
static C++ library)\reqfoot{REQ-uniform-makefile-filename-req}.
...
This chapter states what must already be present on a build host before
\texttt{Bmake It} can be used. For which hosts and languages are in scope
at all, see Section~\ref{sec:platform-scope}.

\section{Build System}

A real BSD make binary is required. \texttt{Bmake It} is built on top of
BSD make's own \verb|bsd.*.mk| infrastructure, or reprogrammed equivalents
where needed; it does not run under GNU
make\reqfoot{CON-use-bsdmake}.

\textbf{[open]} Which package supplies BSD make on each host, and the
exact command to install it, is not yet part of the design record. The
\texttt{*BSD} family ships it as the system default, and macOS reaches it
via MacPorts\reqfoot{CON-cross-platform-targets}; the situation on Linux
is not yet documented here.

\section{Toolchain}

Phase 1 supports two toolchain bundles, selected via
\verb!TOOLCHAIN=<gcc|llvm>! on the \texttt{make} command line; if
unspecified, it defaults to \texttt{llvm}\reqfoot{REQ-toolchain-cli-variable-req-v4}.
At least one of LLVM (Clang) or GCC must be installed and reachable —
see the Phase 1 target table in Section~\ref{sec:platform-scope} for which
combinations are currently targeted.

\textbf{Caveat:} toolchain detection currently locates the compiler via a
single hardcoded absolute path (\verb|/usr/bin/clang|, \verb|/usr/bin/gcc|)
rather than a \verb|PATH| search. On a host where the compiler lives
elsewhere — MacPorts' \verb|/opt/local/bin|, pkgsrc's \verb|/usr/pkg/bin|,
Homebrew, or similar — \verb|TOOLCHAIN=llvm| silently falls back to
whatever plain \texttt{cc} happens to resolve to, with no warning that the
requested toolchain was not actually
used\reqfoot{OBS-toolchain-detection-hardcoded-path-silently-wrong-compiler}.

\section{Languages}

\texttt{Bmake It} compiles C, C++, and lex/yacc grammar sources (which
themselves compile down to C/C++); no other language toolchain is
required\reqfoot{REQ-supported-languages}.

\section{Scope}

\texttt{Bmake It} targets userspace binaries and libraries only — no
kernel module or driver toolchain is
needed\reqfoot{REQ-userland-only-scope}.
...
equivalent wiring was left genuinely unknown\reqfoot{REQ-md-dependency-tracking-freebsd-specific-req}
at the time.

\textbf{Resolved 2026-09-24, and the MacPorts question turned out to be
moot.} Read directly (on a real MacPorts host this time): \verb|mk.prog.mk|/
\verb|mk.lib.mk| never \verb|.include| \verb|bsd.sys.mk|, \verb|bsd.prog.mk|,
\verb|bsd.lib.mk|, or any other real \verb|bsd.*.mk| file at all — only
Bmake It's own \verb|mk.*.mk| role files, each defining fully explicit
\verb|target: source| compile rules with hand-written recipes. So real
\verb|bsd.sys.mk|'s own \verb|-MD| wiring, confirmed or not on any given
host, was never reachable from Bmake It's own compiled objects in the
first place — the question wasn't "does MacPorts have it," it was "does
Bmake It's architecture ever ask." It doesn't. The fallback mentioned
above (add the flags directly) is what was actually built: every gcc/clang
compile recipe now passes \verb|-MMD -MP -MF <obj>.d| itself, and each
object's own \verb|.d| file is conditionally \verb|.include|d on the next
build\reqfoot{REQ-header-dependency-tracking-req}. This is new,
project-owned wiring, not reused \verb|bsd.sys.mk| behavior — confirmed
end-to-end on this MacPorts host: a private header, and a framework-public
header reached only via \verb|PREREQS=| (not the consuming module's own
\verb|src/|), each correctly trigger exactly the affected object to
recompile, and a no-op rebuild recompiles nothing. \verb|TOOLCHAIN=msvc|
has no equivalent yet — \verb|cl.exe|'s own mechanism, \verb|/showIncludes|,
writes to stdout rather than a file, and needs wrapper-side parsing not
built this round.
...
Per-object \verb|-MD| header dependency tracking via \verb|bsd.sys.mk| & \textbf{Inapplicable} — Bmake It's own \verb|mk.prog.mk|/\verb|mk.lib.mk| never \verb|.include| any real \verb|bsd.*.mk| file, so this never applied regardless of host; project now adds \verb|-MMD -MP| itself (gcc/clang only), confirmed working end-to-end \\
\bottomrule
\end{longtable}
\end{center}
